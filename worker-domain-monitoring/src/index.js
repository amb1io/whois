...
async function handleScheduled(env) {
    try {
        ...
        const forcedRecipient = env.FIXED_RECIPIENT_EMAIL?.trim().toLowerCase() ?? null;
        const grouped = groupByEmail(results, forcedRecipient);
        await Promise.allSettled(Array.from(grouped.entries()).map(([email, items]) => sendNotification(env, email, items)));
    }
    catch (error) {
        console.error("[monitor] Failed to process domain notifications:", error);
    }
}
function groupByEmail(rows, forcedRecipient) {
    const map = new Map();
...
async function sendNotification(env, email, domains) {
...
function buildEmailTemplate(email, domains) {
    const rows = domains
        .map((item) => `
        <tr>
